...
::set "FILE_SUBTITLE=%FILE_SUBTITLE:\=/%"
  ...
  echo ffmpeg.exe -hide_banner -threads auto -err_detect ignore_err -y -flags "-output_corrupt" -fflags "+discardcorrupt+autobsf" -i "%FILE_VIDEO%" -c:v libx265 -codec:a copy -crf 26 -preset veryslow -vf "fifo,setpts=PTS-STARTPTS,format=pix_fmts=yuv420p,subtitles=filename='%FILE_SUBTITLE%':force_style='Fontname=Tahoma,Fontsize=28,Shadow=1'" "%FILE_OUT%"
  goto END
